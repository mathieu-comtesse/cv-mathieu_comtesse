...
function el(id){return els[id]??=( {value:id==='drug'?'none':'0',dataset:{},textContent:'',getContext:()=>ctx,getBoundingClientRect:()=>({width:800,height:650}),replaceChildren(){},setPointerCapture(){},focus(){}})}
let frame;const env={document:{getElementById:el,querySelectorAll:()=>[]},window:{addEventListener:(n,fn)=>events[n]=fn},matchMedia:()=>({matches:false}),ResizeObserver:class{observe(){}},devicePixelRatio:1,requestAnimationFrame:fn=>frame=fn,Math};env.window.RoadModel=require('../road-core.js');
...
els['road-start'].onclick();key('keydown','ShiftLeft','Shift');tick(120);key('keyup','ShiftLeft','Shift');assert(+els['road-canvas'].dataset.speed>35);key('keydown','ControlLeft','Control');tick(80);key('keyup','ControlLeft','Control');assert.equal(+els['road-canvas'].dataset.speed,0);
