...
window.addEventListener('keydown',e=>{if(/INPUT|SELECT|BUTTON|TEXTAREA/.test(e.target.tagName))return;const key=e.key.toLowerCase();if(['arrowup','arrowdown','arrowleft','arrowright','z','q','s','d'].includes(key)){e.preventDefault();auto=false;queue=[];$('maze-auto').textContent='Explorer automatiquement';if(key==='q'||key==='arrowleft')maze.facing=(maze.facing+3)%4;else if(key==='d'||key==='arrowright')maze.facing=(maze.facing+1)%4;else step((maze.facing+(key==='s'||key==='arrowdown'?2:0))%4);update();}});
