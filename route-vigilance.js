...
let w=800,h=720,last=0,model=null,paused=false,keys={},cameraY=0,crashAge=0,finished=false,baseline=null,trackSeed=1783;
...
function begin(){model=new RoadRun(trackSeed,settings());c.model=model;paused=false;keys={};finished=false;crashAge=0;cameraY=h*.74;$('road-start').textContent='Mettre en pause';$('road-results').replaceChildren();$('road-status').textContent=model.message;c.focus();}
...
const scale=()=>Math.min(1.5,w/390),ppm=()=>6.6*scale();
...
function render(dt){
 ...
 const desired=h*(.74-Math.min(1,r.speed/180)*.2);cameraY+=(desired-cameraY)*Math.min(1,dt*3);
 g.save();if(r.crashed&&!reduced&&crashAge<.45)g.translate(Math.sin(crashAge*95)*7*(1-crashAge/.45),Math.cos(crashAge*74)*5);
 ...
 vehicle(cx+jitter,cy,{color:'#f4f8fb',kind:'sedan',width:20,length:48,braking:keys.brake},r.crashed?.28:r.heading,r.crashed);
 if(r.crashed){for(let i=0;i<16;i++){const a=i*2.39,rad=Math.min(1,crashAge*2)*40;g.fillStyle=i%2?'#e8954c':'#3c4854';g.fillRect(cx+Math.cos(a)*rad,cy+Math.sin(a)*rad,3,5);}for(let i=0;i<5;i++){g.fillStyle='#34424b66';g.beginPath();g.arc(cx+Math.sin(i)*13,cy-20-i*8-Math.min(crashAge,3)*5,10+i*2,0,7);g.fill();}}
 g.restore();
...
function frame(t){const dt=Math.min(.04,(t-last)/1000||.016);last=t;if(model?.running&&!paused){model.step(dt,{...keys,gas:keys.gas||keys.up});if(!model.running&&!finished)finish();}if(model?.crashed)crashAge+=dt;render(dt);requestAnimationFrame(frame);}c.redraw=()=>render(.016);requestAnimationFrame(frame);
})();
