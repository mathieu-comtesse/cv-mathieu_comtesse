...
class RoadRun{
 ...
 populate(){
  while(this.generated<Math.floor(this.distance/55)+14){
   ...
   for(let side of [-1,1])for(let k=0;k<5;k++)this.scenery.push({d:d+k*11,x:side*(150+this.random()*130),kind:Math.floor(this.random()*6),scale:.7+this.random()*.8,tone:Math.floor(this.random()*3)});
  }
 ...
 step(dt,raw={}){
  ...
  this.x=Math.max(-200,Math.min(200,this.x));if(Math.abs(this.x)>110){this.offroad+=dt;this.speed=Math.max(0,this.speed-35*dt);}
