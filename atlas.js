...
const models=await fetch('atlas-models.json?v=20260923-lagon').then(r=>r.json());
...
function frame(){const dt=Math.min(.05,clock.getDelta()),t=clock.elapsedTime;if(!paused&&!dragging&&!sail.on)spin+=spinVel;
 ...
 {const u=boat.userData,dock=new THREE.Vector3(Math.cos(harbourAngle)*(PIER_R+3.2)-Math.cos(harbourAngle+Math.PI/2)*1.1,-.45,Math.sin(harbourAngle)*(PIER_R+3.2)-Math.sin(harbourAngle+Math.PI/2)*1.1);u.timer-=dt;
 if(u.state==='sail'){boatT+=boatSpeed*dt*60;boat.position.set(Math.cos(boatT*.35)*BOAT_R,-.45+Math.sin(t*2)*.04,Math.sin(boatT*.35)*BOAT_R);boat.rotation.y=-boatT*.35+Math.PI;u.blend=Math.min(1,u.blend+dt*.5);if(u.timer<0&&Math.abs(((boatT*.35)%(Math.PI*2))-harbourAngle)<.25){u.state='in';u.timer=0;}}
 ...
 else{u.timer+=dt;const k=Math.min(1,u.timer/4);const target=new THREE.Vector3(Math.cos(boatT*.35)*BOAT_R,-.45,Math.sin(boatT*.35)*BOAT_R);boat.position.lerp(target,k*.08);u.blend=k;if(k>=1){u.state='sail';u.timer=20+Math.random()*15;}}
