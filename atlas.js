...
const waves=[];for(let i=0;i<34;i++){const w=origami('wave',1,false);w.userData={a:i/34*Math.PI*2+Math.random()*.15,r:8.3+Math.random()*4,speed:.35+Math.random()*.3,scale:.8+Math.random()*.6};scene.add(w);waves.push(w);}
...
function frame(){const dt=Math.min(.05,clock.getDelta()),t=clock.elapsedTime;if(!paused&&!dragging)spin+=spinVel;
 ...
 else{u.timer+=dt;const k=Math.min(1,u.timer/4);const target=new THREE.Vector3(Math.cos(boatT*.35)*10.6,-.45,Math.sin(boatT*.35)*10.6);boat.position.lerp(target,k*.08);u.blend=k;if(k>=1){u.state='sail';u.timer=30+Math.random()*20;}}
 boat.rotation.z=Math.sin(t*1.7)*.06;sails.scale.set(1.1,1.1*(.05+.95*u.blend),1.1);}
