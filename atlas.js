...
const eggs=new Map();let found=0,night=false,paused=false,dragging=null,spin=0,spinVel=.0018,pitch=.62,audio=null,muted=false;
...
function building(step){const g=new THREE.Group();g.position.set(...step.pos);g.userData.step=step;g.add(origami(step.kind));
 ...
 else if(step.kind==='arcade'){
  const log=new THREE.Mesh(new THREE.CylinderGeometry(.16,.16,.3,12),mat('#e0c9a6'));log.position.set(1.05,.15,.5);g.add(edges(log));egg(log,'billot','Le billot de Timber ! — un coup de hache.',()=>{tone([120,90],.12);log.scale.y=.6;setTimeout(()=>log.scale.y=1,300);});
  const cube=box(.26,.26,.26,'#ffd23f',-1.05,.13,.5);g.add(cube);egg(cube,'rubik','Le Rubik’s Cube du banc tourne d’un quart.',()=>{cube.rotation.y+=Math.PI/2;tone([700],.08);});
  const sand=new THREE.Mesh(new THREE.CircleGeometry(.45,20),mat('#ffd9a8'));sand.rotation.x=-Math.PI/2;sand.position.set(-.2,.02,1.1);g.add(sand);egg(sand,'sable','Une flaque de sable : le Sandboard en miniature.',()=>{tone([300,240],.2);sand.material.color.set('#ff9a3d');});
  const hedge=new THREE.Group();for(let i=0;i<5;i++){const h=box(.12,.22,.55,'#8fd39a',1.1+(i%3)*.15,.11,-.5-Math.floor(i/3)*.5,false);hedge.add(h);}g.add(hedge);egg(hedge,'haie','La haie du Labyrinthe, taillée au carré.',()=>{tone([500,600,700,800],.1);hedge.scale.y=1.4;setTimeout(()=>hedge.scale.y=1,400);});}
 const label=makeLabel(step.name);label.position.y=(step.kind==='tower'?3.9:step.kind==='campus'?3.1:2);g.add(label);
...
document.getElementById('world-reset').onclick=()=>{spin=0;pitch=.62;card.hidden=true;setNight(false);};
window.addEventListener('keydown',e=>{if(/INPUT|SELECT|TEXTAREA|BUTTON/.test(e.target.tagName))return;if(e.code==='ArrowLeft')spin-=.08;if(e.code==='ArrowRight')spin+=.08;if(e.code==='Space'){e.preventDefault();document.getElementById('world-pause').click();}});
...
setNight(false);eggsHud.textContent=`Œufs de Pâques : 0 / ${eggs.size}`;canvas.dataset.ready='true';frame();
