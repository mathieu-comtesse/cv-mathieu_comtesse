...
const models=await fetch('atlas-models.json?v=20260923-1').then(r=>r.json());
...
const waves=[];for(let i=0;i<36;i++){const w=origami('wave',1,false);w.castShadow=false;w.material=paperMat.clone();w.material.transparent=true;
 ...
 w.userData={a:i/36*Math.PI*2+Math.random()*.12,d:.6+Math.random()*4.2,speed:.32+Math.random()*.22,scale:.8+Math.random()*.5,phase:'in',age:1+Math.random(),foam};waves.push(w);}
...
function frame(){const dt=Math.min(.05,clock.getDelta()),t=clock.elapsedTime;if(!paused&&!dragging)spin+=spinVel;
 ...
 waves.forEach(w=>{const u=w.userData,f=u.foam;u.age+=dt;
  ...
  else if(u.age>1.1){u.phase='in';u.age=0;u.d=4.2+Math.random()*.9;u.a+=Math.random()*.24-.12;u.scale=.8+Math.random()*.5;}
 ...
 else{u.timer+=dt;const k=Math.min(1,u.timer/4);const target=new THREE.Vector3(Math.cos(boatT*.35)*10.6,-.45,Math.sin(boatT*.35)*10.6);boat.position.lerp(target,k*.08);u.blend=k;if(k>=1){u.state='sail';u.timer=20+Math.random()*15;}}
