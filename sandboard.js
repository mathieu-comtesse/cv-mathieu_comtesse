...
if(stage&&canvas&&reset){
  try{
    ...
    const sculpt=(p,speed)=>{canvas.dataset.drawn='true';const radius=.32+Math.min(speed,15)*.012,r2=radius*radius;for(let i=0;i<position.count;i++){const dx=position.getX(i)-p.x,dy=position.getY(i)-p.y,d=dx*dx+dy*dy;if(d<r2){const f=1-Math.sqrt(d)/radius;const current=position.getZ(i);position.setZ(i,Math.max(base[i]-.34,current-.055*f));}else if(d<r2*2.25){const f=1-Math.abs(Math.sqrt(d)-radius)/(radius*.5);if(f>0)position.setZ(i,Math.min(base[i]+.13,position.getZ(i)+.012*f));}}position.needsUpdate=true;geometry.computeVertexNormals();recolor();burst(p,speed);};
    canvas.addEventListener('pointermove',e=>{const p=hit(e);if(!p)return;tool.visible=true;tool.position.set(p.x,p.y,p.z+.16);const now=performance.now(),speed=lastPoint?Math.min(20,p.distanceTo(lastPoint)/Math.max(8,now-lastTime)*160):0;if(drawing){sculpt(p,speed);sound(true,speed);}lastPoint=p.clone();lastTime=now;});
