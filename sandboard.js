...
if(stage&&canvas&&reset){
  try{
    ...
    canvas.addEventListener('pointermove',e=>{const p=hit(e);if(!p)return;tool.visible=true;tool.position.set(p.x,p.y,p.z+.16);const now=performance.now(),speed=lastPoint?Math.min(20,p.distanceTo(lastPoint)/Math.max(8,now-lastTime)*160):0;if(e.pointerType!=='touch'||drawing){const steps=lastPoint?Math.min(24,Math.max(1,Math.ceil(p.distanceTo(lastPoint)/.10))):1;for(let j=1;j<=steps;j++)sculpt(lastPoint?lastPoint.clone().lerp(p,j/steps):p,speed);sound(true,speed);clearTimeout(sound.stopTimer);sound.stopTimer=setTimeout(()=>sound(false),100);}lastPoint=p.clone();lastTime=now;});
